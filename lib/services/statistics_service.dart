...
class StatisticsService {
  ...
  Future<Map<String, dynamic>> getDashboardStats() async {
    ...
    for (final a in localAssessments) {
      ...
      if (a.assessmentDate.isAfter(thisWeek) ||
          a.assessmentDate.isAtSameMomentAs(thisWeek)) {
        weekLocal++;
      }
      if (a.assessmentDate.isAfter(thisMonth) ||
    ...
    for (final assessment in dsm5Assessments) {
      ...
      if (assessmentDate.isAfter(thisWeek) ||
          assessmentDate.isAtSameMomentAs(thisWeek)) {
        weekDsm5++;
      }
      if (assessmentDate.isAfter(thisMonth) ||
          assessmentDate.isAtSameMomentAs(thisMonth)) {
        monthDsm5++;
      }
      ...
      capacityStats['DSM-5: $severity'] =
          (capacityStats['DSM-5: $severity'] ?? 0) + 1;
    }
    ...
    for (final assessment in mhcaAssessments) {
      final dateStr =
          assessment['created_at'] ?? assessment['assessment_date'] ?? '';
      final assessmentDate = DateTime.tryParse(dateStr) ?? now;
      final dateOnly = DateTime(
        assessmentDate.year,
        assessmentDate.month,
      ...
      final determination =
          (assessment['determination'] ?? 'Unknown') as String;
      if (determination.isNotEmpty) {
        capacityStats['MHCA: $determination'] =
            (capacityStats['MHCA: $determination'] ?? 0) + 1;
      }
    }
    ...
    for (final a in apiAssessments) {
      ...
      localByDate[dateOnly] = (localByDate[dateOnly] ?? 0) + 1;
      capacityStats[a.overallCapacity] =
          (capacityStats[a.overallCapacity] ?? 0) + 1;
    }
    ...
    return {
      ...
      'averagePerDay': totalAssessments > 0
          ? (totalAssessments / 7).toStringAsFixed(1)
          : '0',
    };
  }
  ...
  Future<List<Map<String, dynamic>>> getMonthlyTrends() async {
    ...
    for (final assessment in allAssessments) {
      final monthKey =
          '${assessment.assessmentDate.year}-${assessment.assessmentDate.month.toString().padLeft(2, '0')}';
      ...
      if (capacity.contains('has capacity')) {
        monthlyData[monthKey]!['hasCapacity'] =
            monthlyData[monthKey]!['hasCapacity']! + 1;
      } else if (capacity.contains('lacks capacity') ||
          capacity.contains('needs 100%')) {
        monthlyData[monthKey]!['lacksCapacity'] =
            monthlyData[monthKey]!['lacksCapacity']! + 1;
      } else {
    ...
    for (final assessment in mhcaAssessments) {
      ...
      final determination = (assessment['determination'] ?? '')
          .toString()
          .toLowerCase();
  ...
  Future<Map<String, dynamic>> getAssessorPerformance(
    String assessorName,
  ) async {
    ...
    final userAssessments = allAssessments
        .where((a) => a.assessorName == assessorName)
        .toList();
    ...
    final daysSinceFirst =
        DateTime.now().difference(firstAssessment).inDays + 1;
    final averagePerWeek = (userAssessments.length / daysSinceFirst * 7)
        .toStringAsFixed(1);
