...
class AssessmentQuestions {
  ...
  static Map<String, dynamic> calculateCapacityScore(
    ...
    for (var i = 0; i < entries.length; i++) {
      ...
      final question = _questionForEntry(
        key: entry.key,
        index: i,
  ...
  static Map<String, int> calculateDomainScores(
    Map<String, dynamic> responses,
  ) {
    Map<String, int> domainScores = {};
    final questions = getStandardQuestions();
  ...
  /// Calculate the highest item score in each DSM-5 domain.
  ///
  /// DSM-5 Level 1 follow-up thresholds are based on item rating severity
  /// inside a domain, not the sum of all items in that domain.
  static Map<String, int> calculateDomainHighestScores(
    Map<String, dynamic> responses,
  ) {
  ...
  static List<String> getDomainsRequiringFollowUp(
    Map<String, int> domainScores,
  ) {
  ...
  static List<String> getRecommendations(
    ...
    domainScores.forEach((domain, score) {
      ...
      if (domain.contains('Suicidal') && score >= 1) {
        ...
        recommendations.add(
          'IMPORTANT: Consider psychiatric evaluation for psychotic symptoms - detailed assessment recommended',
        );
      } else if (domain.contains('Substance') && score >= 1) {
        recommendations.add(
          'IMPORTANT: Assess substance use patterns - detailed assessment recommended',
        );
      } else if (domain.contains('Depression') && score >= 4) {
        recommendations.add(
          'Screen for depressive disorder (PHQ-9 recommended)',
        );
  ...
  static List<String> getDomainsNeedingLevel2(Map<String, dynamic> responses) {
    final domainHighestScores = calculateDomainHighestScores(responses);
  ...
  static Question? _questionForEntry({
    ...
    for (final question in questions) {
      if (question.questionId == key) return question;
    }

    // Backward compatibility for older records that used unstable hash keys.
    if (entriesLength == questions.length && index < questions.length) {
  ...
  static int _scoreResponse(dynamic value, List<String> options) {
    if (value is int) return value.clamp(0, 4);
    if (value is Map) {
      return _scoreResponse(value['answer'], options);
    }

    final index = options.indexOf(value.toString());
